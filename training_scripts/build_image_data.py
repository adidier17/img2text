...
tf.flags.DEFINE_string("train_captions", "",
                       "Train images unprocessed captions.")
tf.flags.DEFINE_string("train_processed_captions", "",
                       "Training captions JSON file (where the processed info will go.")
tf.flags.DEFINE_string("test_image_dir", "",
                       "Test image directory.")
tf.flags.DEFINE_string("test_captions", "",
                       "Test images unprocessed captions.")
tf.flags.DEFINE_string("test_processed_captions", "",
                       "Test captions JSON file (where the processed info will go).")
...
def _generate_annotations():
    ...
    def _load_caption_dict(token_path):
        caption_dict = dict()

        with open(token_path, "r") as f:
            for line in f:
                cap_break_index = line.index('.jpg') + 5
    ...
    def _write_to_json(caption_dict, image_dir, out_file):
        ...
        data_set['file_caps'] = caption_dict

        with open(out_file, 'w') as data_set_file:
...
def main(unused_argv):
    ...
    train_dataset = _load_and_process_metadata(
        FLAGS.train_processed_captions,
        FLAGS.train_image_dir)
    test_dataset = _load_and_process_metadata(
        FLAGS.test_processed_captions,
    ...
    _process_dataset("val", test_dataset, vocab, FLAGS.val_shards)
